...
function setTxMessage(text, type) {
  ...
  if (type === "success") {
    ...
    el.style.color = "#FFFFFF"; // ขาว หรือ default
  }
}

/* ===================== ADD THBC TO WALLET ==================== */
/**
 * เรียกให้ Wallet โชว์ popup เพิ่ม token THBC
...
async function addTHBCToWallet() {
  try {
    ...
    // TODO: เปลี่ยนเป็น URL รูปเหรียญจริงของ THBC
    ...
    if (wasAdded) {
      ...
      console.log("User rejected adding THBC token");
    }
  } catch (err) {
    console.error("wallet_watchAsset error:", err);
    // ไม่ต้องขึ้น error แรง ๆ ก็ได้ เก็บไว้ดูใน console พอ
  }
}
...
async function connectWallet() {
  try {
    ...
    // ใช้ provider.send แทน injected.request เพื่อรองรับ Binance / Bitget
    ...
    // เช็ค network เป็น BSC mainnet (56)
    ...
    // subscribe event เปลี่ยนบัญชี / เปลี่ยน chain
...
async function onBuyTHBC() {
  ...
  try {
    ...
    const usdtAmount = ethers.utils.parseUnits(amountStr, 18); // USDT 18 decimals
    ...
    // เสนอให้เพิ่ม THBC เข้า wallet ทันทีหลังซื้อสำเร็จ
    addTHBCToWallet();
  } catch (err) {
