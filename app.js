...
/**
 * เลือก provider ให้รองรับหลาย wallet:
 * - ถ้ามี window.ethereum ใช้ก่อน (MetaMask / Bitget / Binance หลายตัวแชร์อันนี้)
 * - ถ้าไม่มีลอง BinanceChain
 * - แล้วค่อยดู bitget / bitkeep แบบเก่า
 */
function getInjectedProvider() {
  // ส่วนใหญ่กระเป๋าสมัยใหม่จะ inject เป็น window.ethereum
  if (window.ethereum) {
    return window.ethereum;
  }

  // Binance Wallet บางเวอร์ชันใช้ window.BinanceChain
  if (window.BinanceChain) {
    return window.BinanceChain;
  }

  // Bitget (บางเวอร์ชันเก่า)
...
/* ==================== ADD THBC TO WALLET ===================== */
/** เรียกหลังจากซื้อสำเร็จ เพื่อให้ wallet ถามเพิ่ม THBC ให้อัตโนมัติ */
async function addTHBCToWallet() {
  try {
    ...
    // เปลี่ยนลิงก์รูปเป็นของจริงได้ภายหลัง
    ...
    // ไม่ต้องขึ้น error ก็ได้ ให้เงียบ ๆ
  }
}
...
async function init() {
  ...
  // ถ้าคุณอยากมีปุ่มแยก "Add THBC to Wallet" ก็สร้าง <button id="btnAddTHBC">
...
async function onBuyTHBC() {
  ...
  try {
    ...
    // เสร็จแล้วลองให้ wallet ถามเพิ่ม THBC auto
    addTHBCToWallet();
  } catch (err) {
